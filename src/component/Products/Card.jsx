      ...
      <div className="mt-6 space-y-3">
        {product.features.map((feature, index) => (
          <div key={index} className="flex items-center gap-3">
            ...
            <p className="text-[16px] leading-none text-[#667085]">{feature}</p>
          </div>
        ))}
      </div>

      <button
        className={`mt-8 h-[54px] w-full rounded-full ${selected ? "bg-green-600" : "bg-primary"} text-[18px] font-semibold text-white`}
        onClick={onclickhandler}
        disabled={selected}
      >
        {selected ? '✓ Add to Cart' : 'Buy Now'}
      </button>
    </div>
  );
};
